...
export default function GoErasmusPage() {
  ...
  const [uczelnieZagr, setUczelnieZagr] = useState([]);
  const [wybranyKierunek, setWybranyKierunek] = useState('');
  const [wybranySemestr, setWybranySemestr] = useState('');
  const [wybranaUczelnia, setWybranaUczelnia] = useState('');
  const [czyLaduje, setCzyLaduje] = useState(true);

  useEffect(() => {
    const fetchDane = async () => {
      const { data } = await supabase.from('przedmioty').select('uczelnia_nazwa, kierunek_nazwa');
      if (data) {
        const uek = data.filter(p => p.uczelnia_nazwa === 'UEK');
        const zagr = data.filter(p => p.uczelnia_nazwa !== 'UEK');
        setKierunkiUEK([...new Set(uek.map(i => i.kierunek_nazwa).filter(Boolean))].sort());
        setUczelnieZagr([...new Set(zagr.map(i => i.uczelnia_nazwa))].sort());
      }
      setCzyLaduje(false);
    };
    fetchDane();
  ...
  const handleWyszukaj = (e) => {
    e.preventDefault();
    navigate(`/sylabusy?kierunek=${encodeURIComponent(wybranyKierunek)}&semestr=${wybranySemestr}&uczelnia=${encodeURIComponent(wybranaUczelnia)}`);
  };

  const czyZablokowany = !wybranyKierunek || !wybranySemestr || !wybranaUczelnia;

  return (
    <div className="w-full flex flex-col lg:flex-row items-center gap-16">
      
      {/* LEWA: Formularz Zdecydowanego */}
      <div className="w-full lg:w-1/2 flex flex-col">
        <form onSubmit={handleWyszukaj} className="flex flex-col gap-10">
          <div className="flex flex-col gap-5">
            <h2 className="text-2xl font-black text-[#a420eb] uppercase tracking-widest">Uczelnia Macierzysta</h2>
          ...
          <div className="flex flex-col gap-5">
            <h2 className="text-2xl font-black text-gray-400 uppercase tracking-widest">Uczelnia Partnerska</h2>
            <select value={wybranaUczelnia} onChange={(e) => setWybranaUczelnia(e.target.value)} disabled={czyLaduje} className="w-full bg-white border border-gray-200 rounded-lg p-3.5 text-sm text-gray-800 outline-none focus:border-[#a420eb] transition-all">
              <option value="" disabled>-- Wybierz docelową uczelnię --</option>
              {uczelnieZagr.map(u => <option key={u} value={u}>{u}</option>)}
            </select>
          </div>
